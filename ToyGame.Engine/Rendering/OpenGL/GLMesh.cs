...
namespace ToyGame.Rendering.OpenGL
{
  ...
  internal sealed class GLMesh
  {
    ...
    public GLMesh(RenderCore renderCore, Vector3[] positions, uint[] indexes, Vector3[] normals = null,
      ...
      BufferUsageHint usage = BufferUsageHint.StaticDraw)
    {
      renderCore.AddResourceLoadAction(() =>
      {
        ...
        IndexBuffer = GLVertexBufferObject.FromData(renderCore, indexes, BufferTarget.ElementArrayBuffer, usage);
        var vbos = new List<GLVertexBufferObject>();
        vbos.Add(GLVertexBufferObject.FromData(renderCore, positions, BufferTarget.ArrayBuffer, usage,
          new GLVertexAttribute("position", 3, VertexAttribPointerType.Float, false, Vector3.SizeInBytes, 0)));
        if (normals != null)
        {
          vbos.Add(GLVertexBufferObject.FromData(renderCore, normals, BufferTarget.ArrayBuffer, usage,
            new GLVertexAttribute("normal", 3, VertexAttribPointerType.Float, false, Vector3.SizeInBytes, 0)));
        }
        if (uv0 != null)
        {
          vbos.Add(GLVertexBufferObject.FromData(renderCore, uv0, BufferTarget.ArrayBuffer, usage,
            new GLVertexAttribute("uv0", 2, VertexAttribPointerType.Float, false, Vector2.SizeInBytes, 0)));
        }
        if (uv1 != null)
        {
          vbos.Add(GLVertexBufferObject.FromData(renderCore, uv1, BufferTarget.ArrayBuffer, usage,
            new GLVertexAttribute("uv1", 2, VertexAttribPointerType.Float, false, Vector2.SizeInBytes, 0)));
        }
        if (colors != null)
        {
          vbos.Add(GLVertexBufferObject.FromData(renderCore, colors, BufferTarget.ArrayBuffer, usage,
